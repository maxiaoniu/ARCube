...
GLTextureCube::GLTextureCube(const QStringList& fileNames, int size)
{
    ...
    foreach (QString file, fileNames) {
        ...
        // Works on x86, so probably works on all little-endian systems.
        // Does it work on big-endian systems?
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + index, 0, GL_RGB, image.width(), image.height(), 0,
            GL_RGB, GL_UNSIGNED_BYTE, image.bits());
    ...
    while (index < 6) {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + index, 0, GL_RGB, size, size, 0,
            GL_RGB, GL_UNSIGNED_BYTE, 0);
        ++index;
    }
